...
describe('SSR.processHtml', async () => {
  ...
  it('should preserve <pre>/<code> content in processHtml without token resolution or custom tag rendering', async () => {
    ...
    assert.ok(result.includes('{{someToken}}'), `Expected raw "{{someToken}}" preserved: ${result}`);
    assert.ok(result.includes('{[items]}'), `Expected raw "{[items]}" preserved: ${result}`);
    assert.ok(result.includes('&lt;proc-basic&gt;&lt;/proc-basic&gt;'), `Expected escaped <proc-basic> preserved: ${result}`);
    assert.ok(!result.includes('processed'), `Must NOT render custom tags inside <pre>/<code>: ${result}`);
  });
